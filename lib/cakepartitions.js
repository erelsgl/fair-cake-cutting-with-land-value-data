...
module.exports = {
	...
	/**
	 * @return the average, over all agents, of the largest relative envy the agent feels towards another agent
	 */
	meanEnvy: function(valueFunctions) {
		if (!valueFunctions.length) return 0; // default mean value for empty array
		return _.reduce(valueFunctions, function(memo, enviousValueFunction) {
			var envyOfMe = module.exports.largestEnvyOfAgent(enviousValueFunction, valueFunctions);
			return memo + envyOfMe;
		},0) / valueFunctions.length;
	}
}
